...
${safeText}
    ...
    // ============================
    // FILE (Link-Based One-Time)
    // ============================

    if (content.type === "file") {

      // If one-time → consume link immediately
      if (content.oneTimeView) {
        await Content.deleteOne({ _id: content._id });
      }
...
exports.downloadFile = async (req, res) => {
  try {
    ...
    // Since one-time link already deleted record,
    // we must fetch file by uniqueId from request param
    const content = await Content.findOne({ uniqueId: req.params.id });
    ...
    if (content) {
      ...
      return res.download(filePath, content.originalFileName);
    }

    // If one-time link already consumed,
    // file might still exist — allow download once
    const uploadsDir = "uploads";
    const file = fs.readdirSync(uploadsDir)
      .find(f => f.includes(req.params.id));
